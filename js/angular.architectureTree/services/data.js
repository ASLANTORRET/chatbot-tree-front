...
angular.module('ChartsApp').service('data', function ($http, $q, bus) {
    ...
    var jsonData;
    var image = {};
    //var url = "http://localhost:8081/BotAdmin-1.0/json";
    var url = location.href.substring(0, location.href.lastIndexOf("/")) + "/json";
    // var url = "/BotAdmin-1.0/json";
    ...
    var postJsonData = function () {
        var jdata = getJsonData();

        console.log("request data string: " + JSON.stringify(jdata));
        ...
        if(!angular.equals(getImage(), {})){
            data.file1 = getImage().file1;
            data.filename1 = getImage().name1;
            data.file2 = getImage().file2;
            data.filename2 = getImage().name2;
            removeImage();
        }
        ...
        .then(function(response) {
                // success
            var responseText = "Данные обновлены удачно!";
            if(response.data != "ok"){
                responseText = "Ошибка при сохранении"
            }
    ...
    // var postImageFile = function(image){
    //     $http({
    //         url: "http://localhost:8081/BotAdmin-1.0/image",
    //         method: "POST",
    //         data: image
    //     })
    //     .then(function(response) {
    ...
    var setImage = function(file, name, index){
        if(index == 1){
            image.file1 = file;
            image.name1 = name;
        }
    ...
    var addNode = function(name, newNode) {
        newNode = newNode || { name: 'Новая ветка' };
        var node = getNodeByName(name);
        if (!node.children) {
            node.children = [];
        }
